...
export class Ctranslate2Transcriber extends OpenaiTranscriber {
  ...
  async transcribe ({
    ...
    if (computeType && this.engine.validComputeTypes && !this.engine.validComputeTypes.includes(computeType)) {
      const validTypesStr = this.engine.validComputeTypes.join(', ')
      this.logger.warn(`Invalid computeType "${computeType}". Expected one of: ${validTypesStr}`)
      computeType = null
    }
